...
> the command plane, ordered/deduped/gap-replayed `.log` delivery, and the interaction handle
> (`send` / `signal` / `metadata`). Offer-ring, call/media, recording, transfer, and the
...
`send()` is the **only** way to record a fact: it publishes a command on `interaction.<id>.cmd`
