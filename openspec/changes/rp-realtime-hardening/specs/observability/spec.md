# Delta for Observability — realtime hardening

Adds a health/readiness surface to every RP binary (RH-06) and wires the dormant OTLP export +
a metrics surface (RH-09) so the realtime plane's failure modes (DLQ growth, lease flaps, roster
5xx, fan-out lag) become probeable and alertable rather than log-only. Builds on the trace seam from
`m1_5-f5b-rp-trace-continuity` (`obs.InitTracer` is a no-op unless `OTEL_EXPORTER_OTLP_ENDPOINT` is
set).
