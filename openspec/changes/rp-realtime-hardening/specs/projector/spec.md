# Delta for Projector (Participation/Fan-out service) — realtime hardening

Anchors the previously-dangling `RDL-01`/`RDL-02` ids (the concurrent-fan-out perf commit, tagged in
tests but defined in no spec) and ADDS the corrected lease-fencing behaviour (`RDL-03`), the
`Nak`→DLQ delivery guard, the roster soft-fail/no-cache rules, the HA-replica posture, and the
`fanoutConcurrency` config surface. The service tails the SHARED `tenant.*.interaction.*.log`
(`INTERACTION_LOGS`) and writes `tenant.<tid>.agent.<aid>.feed.>` (`AGENT_FEED`) + the DLQ
`tenant.<tid>.agent.dlq.feed`; it holds a single-active KV leader **lease**. Owned ports (`LogSource`,
`FeedSink`, `LeaseStore`, `Roster`, `SnapshotStore`) only — no concrete NATS in the core.
